...
TEST(Test_WordIndex, Simple) {
  ...
  auto res2_ = index.lookup_word(apples);
  auto it_ = res2_.begin();
  ASSERT_EQ(doc_name1, it_->doc_name);
  ASSERT_EQ(3, it_->rank);
  ...
  index.record(grapes, doc_name1);
  index.record(grapes, doc_name3);
  vector<string> q7 {pears, grapes};
  auto res7 = index.lookup_query(q7);
  ASSERT_EQ(1U, res7.size());
  it = res7.begin();
  ASSERT_EQ(doc_name1, it->doc_name);
  ASSERT_EQ(2, it->rank);
