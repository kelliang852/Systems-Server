...
// read into a string, go through the string
// use word index to build look up table recording frequency of word in the string
// index stored in output parameter, storing words and frequency 
// hint: boost functions will be used ->trim or replaceall or split
// need both filereader and wordindex
static void handle_file(const string& fpath, WordIndex *index) {
  // TODO: implement
  // cout<<fpath<<endl;
  ...
  boost::split(vecToks, contents, !boost::is_alpha());

  // while ((pos = contents.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")) != std::string::npos) {
  //   if (pos!=0){
  //     token = contents.substr(0, pos);
  //     vecToks.push_back(token);
  //   }
  //   // erase token + delimiter
  //   contents.erase(0, pos+1);  
  // }
  // if (contents.length()>0) {
  //   vecToks.push_back(contents);
  // } 

  // cout<<"vecTok size = "<<vecToks.size()<<endl;
  for (auto& i :vecToks) { 
    boost::trim(i);
    boost::to_lower(i);
    if (i.length() > 0) {
  ...
  // cout<<"index size = "<<index->num_words()<<"\n"<<endl;
